...
def registrarse():
    ...
    if not comprobar_contraseña(contraseña_us):
        return jsonify(
            {
                "mensaje": "Ponga una contraseña mas segura (8 caracteres, 3 mayusculas, 3 minusculas, 2 numeros)",
                "tipo": "danger",
            }
        )
...
def validar_codigo():
    ...
    if check_password_hash(
        session["codigo_validacion"],
        f"{session['datos_temporales']['correo_usuario']}{codigo}",
    ):
        ...
        with db.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            ...
            try:
                if not check_password_hash(
                    usuario[0]["contraseña_usuario"], contraseña_encriptada
                ):
                    return jsonify(
                        {"mensaje": "Contraseña incorrecta", "tipo": "danger"}
                    )
            except AttributeError:
                return jsonify(
                    {"mensaje": "Inicia sesion por google", "tipo": "danger"}
                )
...
def cambiar_contraseña():
    ...
    if contraseña_nueva != contraseña_nueva_confirmacion:
        return jsonify(
            {"mensaje": "Las contraseñas nuevas no coinciden", "tipo": "danger"}
        )
    ...
    actualizar_datos(
        "USUARIOS",
        {"contraseña_usuario": contraseña_nueva_encriptada},
        {"correo_usuario": correo},
    )
    session.pop("cambiar_contraseña_usuario")
    return jsonify(
        {
            "redirigir": "/iniciar_sesion",
            "mensaje_redirigir": {
                "mensaje": "Contraseña cambiada exitosamente, inicia sesión de nuevo",
                "tipo": "success",
            },
        }
    )
...
def iniciar_google():
    ...
    if dato_en_db(email, "correo_usuario"):
        ...
        mensaje = registrarse_google(
            id_info["name"], email, id_info["sub"], id_info["picture"]
        )
    return mensaje
...
def cerrar_sesion():
    ...
    if not usuario_actual:
        ...
        return jsonify(
            {
                "redirigir": "/",
                "mensaje_redirigir": {"mensaje": "Sesión cerrada", "tipo": "success"},
            }
        )
    with conectar() as db:
        with db.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            ip = obtener_ip()
            cursor.execute(
                'SELECT ip_usuario FROM "USUARIOS" WHERE correo_usuario = %s',
                (usuario_actual["correo_usuario"],),
            )
            ips = cursor.fetchone()
            hash = None
            for ip_usuario in ips["ip_usuario"]:
                if check_password_hash(ip_usuario, ip):
                    ...
                    break
            cursor.execute(
                'UPDATE "USUARIOS" SET ip_usuario = array_remove(ip_usuario, %s) WHERE correo_usuario = %s',
                (hash, usuario_actual["correo_usuario"]),
            )
            session.clear()
    return jsonify(
        {
