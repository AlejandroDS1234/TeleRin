...
from servidor.services.servicios_historias import (
    verificar_nommbre_historia,
    verificar_descripcion_historia,
    verificar_saga,
    verificar_historia,
    verificar_id,
    obtener_info_historia,
)
...
def hashtag_db(texto: str, id: str, tabla_campo: str):
    ...
    for hashtag in hashtags:
        ...
        index_hashtag = dato_en_db(hashtag, "nombre_hashtag", "hashtags")[0][
            "id_hashtag"
        ]
        insertar_db(
            tabla_campo["tabla"],
            {tabla_campo["campo"]: id, "id_hashtag": index_hashtag},
        )
    return True
...
def crear_historia():
    ...
    with conectar() as db:
        with db.cursor() as cursor:
            cursor.execute(
                """SELECT publicada FROM "historias" WHERE id_historia = %s""",
                (id_historia,),
            )
    ...
    actualizar_datos(
        "historias",
        {
            ...
            "id_saga": saga_historia,
            "contenido_historia": historia,
            "idioma": idioma,
            "visibilidad_historia": visivilidad_historia,
            "publicada": True,
            "borrador_historia": None,
        },
    ...
    hashtag_db(
        ...
        id_historia,
        {"tabla": "hashtags_historias", "campo": "id_historia"},
    )
    ...
    actualizar_documento(
        ...
        obtener_usuario_codigo(usuario["codigo_usuario"]),
    )
    return jsonify(
        {
            "redirigir": redirigir,
            "mensaje_redirigir": {"mensaje": mensaje, "tipo": "success"},
            "id_historia": id_historia,
        }
    )
...
def guardar_borrador_historia():
    ...
    with conectar() as db:
        with db.cursor() as cursor:
            cursor.execute(
                "SELECT id_historia FROM historias WHERE id_historia = %s AND codigo_usuario = %s",
                (id_historia, usuario_actual["codigo_usuario"]),
            )
            historia_usuario = cursor.fetchall()
            if not historia_usuario:
                return jsonify(
    ...
    actualizar_datos(
        "historias",
        {"borrador_historia": borrador_historia},
        {"id_historia": id_historia},
    )
...
def calificar_historia(id_historia):
    ...
    if calificacion == 0:
        if dato_en_db(
            None,
            {"id_historia": id_historia, "codigo_usuario": id_usuario},
            "calificacion_historia",
        ):
            with conectar() as db:
                with db.cursor() as cursor:
                    cursor.execute(
                        'DELETE FROM "calificacion_historia" WHERE id_historia = %s AND codigo_usuario = %s',
                        (id_historia, id_usuario),
                    )
                    db.commit()
...
def continuar_historia():
    ...
    with conectar() as db:
        with db.cursor(cursor_factory=psycopg2.extras.RealDictCursor) as cursor:
            cursor.execute(
                """SELECT borrador_historia, id_historia FROM "historias" WHERE codigo_usuario = %s AND publicada = FALSE""",
                (usuario_actual["codigo_usuario"],),
            )
...
def eliminar_borrador(id_historia):
    ...
    with conectar() as db:
        with db.cursor() as cursor:
            cursor.execute(
                """SELECT publicada FROM "historias" WHERE id_historia = %s AND codigo_usuario = %s""",
                (id_historia, usuario_actual["codigo_usuario"]),
            )
            publicada = cursor.fetchone()
            ...
            if publicada[0]:
                cursor.execute(
                    """UPDATE historias SET borrador_historia = NULL WHERE id_historia = %s AND codigo_usuario = %s""",
                    (id_historia, usuario_actual["codigo_usuario"]),
                )
                db.commit()
                return jsonify(
                ...
                cursor.execute(
                    """DELETE FROM historias WHERE id_historia = %s AND codigo_usuario = %s""",
                    (id_historia, usuario_actual["codigo_usuario"]),
                )
