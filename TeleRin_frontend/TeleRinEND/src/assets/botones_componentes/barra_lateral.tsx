...
function Navbar() {
  ...
  return (
    <div className="flex top-0 left-0 w-full h-16 bg-white/30 backdrop-blur-md items-center justify-around z-50 shadow-md">

      <button className="hover:cursor-pointer" onClick={() => {navigate("/perfil"); setResaltado("/perfil")}}>
        <img className="h-10 aspect-square rounded-full border-2" style={{ borderColor: (resaltado == "/perfil"? "#FF0000": "#000") }} src={`http://localhost:1240/Fotos/perfil/${usuario.foto_perfil_usuario}`} />
      </button>
      ...
      <button className="hover:cursor-pointer" onClick={() => navigate("/crear_publicacion")}>
        <Feather color={resaltado == "/"? "#FF0000": "#000"} />
      </button>
    </div>
  );
} 
