import "./App.css";
import LogoSintexto from "./static/LogoSintexto.svg";
import {useEffect} from "react";



function Estructura() {
useEffect(() => {
  fetch("http://localhost:3000/api/users")
    .then(response => response.json())
    .then(data => console.log(data))
    .catch(error => console.error("Error fetching data:", error));
});
return (
<>
  <div className="contenedor">
    <header className="header">
      <div className="logo" style={{ width: "max-content, height: 90%" }}>
        <img src={LogoSintexto} alt="Logo TeleRin" />

        <h1>TeleRin</h1>
      </div>
      <div className="opciones"></div>
    </header>
    <footer className="footer">
      <div className="contenido_footer"></div>
      <div id="fecha_actualizacion" />
    </footer>
  </div>
  <div className="mensaje_flotante mensaje-{{ category }}">
    {"{"}
    {"{"} message {"}"}
    {"}"}
  </div>
</>
)
}


function menuLateral() {
  return (
    <div id="menu_lateral">
  <ul>
    <li id="inicio">
      <a href="{{ url_for('inicio') }}">
        <svg
          width="100%"
          height="100%"
          viewBox="0 0 16 16"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M1 6V15H6V11C6 9.89543 6.89543 9 8 9C9.10457 9 10 9.89543 10 11V15H15V6L8 0L1 6Z" />
        </svg>
      </a>
    </li>
    <li id="perfil">
      <a href="{{ url_for('perfil') }}">
        <svg
          width="100%"
          height="100%"
          viewBox="0 0 32 32"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path d="M16 15.503A5.041 5.041 0 1 0 16 5.42a5.041 5.041 0 0 0 0 10.083zm0 2.215c-6.703 0-11 3.699-11 5.5v3.363h22v-3.363c0-2.178-4.068-5.5-11-5.5z" />
        </svg>
      </a>
    </li>
    <li id="crear_historia">
      <a href="{{ url_for('crear_historia') }}">
        <svg
          width="100%"
          height="100%"
          viewBox="0 0 24 24"
          xmlns="http://www.w3.org/2000/svg"
        >
          <path opacity="0.15" d="M4 20H8L18 10L14 6L4 16V20Z" />
          <path
            d="M12 20H20.5M18 10L21 7L17 3L14 6M18 10L8 20H4V16L14 6M18 10L14 6"
            strokeWidth="1.5"
            strokeLinecap="round"
            strokeLinejoin="round"
          />
        </svg>
      </a>
    </li>
    <li id="cerrar_sesion">
      <a href="{{ url_for('cerrar_sesion') }}">
        <svg
          fill="#000000"
          height="100%"
          width="100%"
          version="1.1"
          id="Icons"
          xmlns="http://www.w3.org/2000/svg"
          xmlnsXlink="http://www.w3.org/1999/xlink"
          viewBox="0 0 32 32"
          xmlSpace="preserve"
        >
          <path d="M30.9,13.6c-0.1-0.1-0.1-0.2-0.2-0.3l-4-4c-0.4-0.4-1-0.4-1.4,0s-0.4,1,0,1.4l2.3,2.3H22v-3V3c0-0.6-0.4-1-1-1H4c0,0,0,0,0,0C3.9,2,3.7,2,3.6,2.1c0,0,0,0-0.1,0c0,0-0.1,0-0.1,0.1c0,0-0.1,0.1-0.1,0.1c0,0,0,0,0,0C3.2,2.4,3.1,2.5,3.1,2.6c0,0,0,0,0,0.1C3,2.8,3,2.9,3,3v22c0,0.4,0.2,0.8,0.6,0.9l9,4C12.7,30,12.9,30,13,30c0.2,0,0.4-0.1,0.5-0.2c0.3-0.2,0.5-0.5,0.5-0.8v-3h7c0.6,0,1-0.4,1-1V15h5.6l-2.3,2.3c-0.4,0.4-0.4,1,0,1.4c0.2,0.2,0.5,0.3,0.7,0.3s0.5-0.1,0.7-0.3l4-4c0.1-0.1,0.2-0.2,0.2-0.3C31,14.1,31,13.9,30.9,13.6z M10,21c0,0.6-0.4,1-1,1s-1-0.4-1-1v-4c0-0.6,0.4-1,1-1s1,0.4,1,1V21z M20,10v14h-6V7c0-0.4-0.2-0.8-0.6-0.9L8.7,4H20V10z" />
        </svg>
      </a>
    </li>
  </ul>
</div>

  )
}

function inicio() {
  return (<h1>Hola mundo</h1>)

}

export default menuLateral; Estructura; inicio;